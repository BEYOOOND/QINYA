...
跨平台红包封面/气泡挂件编辑器，支持视频帧裁切、封面图外挂反选、独立气泡挂件排版。本文件为开发入口，汇总核心能力、模块分布与常用流程。
...
| 反选外挂 | “封面图外挂”自动生成环形 mask（A/B 两区），A 区保留主画面+所有素材，B 区仅保留挂件模块素材。 |
...
3. 在素材面板切换到“封面图挂件”模块，添加外挂素材
4. A 区（挂件→红包）保留主画面与所有素材，B 区（红包→不可编辑）仅保留外挂素材
5. 导出文件命名为“封面图外挂”并放入专属子目录
